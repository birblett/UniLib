...
UniLib.insert_in_method_before(:PokeBattle_AI, :pbTypeModNoMessages, target,
  ...
  opponent.ability_event_value(:fake_reduce_weakness) { |arr| typemod /= 2 if check_type(type, arr, UniLib::TYPE_WEAKNESS_MAP) }
  opponent.ability_event_value(:fake_resistance) { |arr| typemod /= 2 if check_type(type, arr, UniLib::TYPE_RESISTANCE_MAP) }
  opponent.apply_ability_event(:type_effectiveness_simple, opponent, type, false) { |m| typemod *= m }
...
UniLib.insert_in_method(:PokeBattle_Battler, :__shadow_pbInitPokemon, "crestStats if @crested",
  ...
  self.apply_ability_event(:battle_stat_calc, self, stats) {}
...
UniLib.insert_in_method_before(:PokeBattle_Move, :pbCalcDamage, "case attacker.ability",
  "attacker.apply_ability_event(:damage_mod, attacker, opponent, self, hitnum, nil) { |m| basemult *= m }
  opponent.apply_ability_event(:damage_taken_mod, opponent, attacker, self, hitnum, nil) { |m| basemult *= m }")

# move damage/damage taken modifier (ai)
UniLib.insert_in_method_before(:PokeBattle_AI, :pbRoughDamage, "typecrest = false",
  "attacker.apply_ability_event(:damage_mod, attacker, opponent, move, move.pbNumHits(attacker), self) { |m| damage *= m }
  opponent.apply_ability_event(:damage_taken_mod, opponent, attacker, move, move.pbNumHits(attacker), self) { |m| damage *= m }")
...
UniLib.insert_in_method(:PokeBattle_AI, :pbTypeModNoMessages, "id = move.move",
  "attacker.apply_ability_event(:move_type_override, attacker, move, type) { |m| type = m }")
...
UniLib.insert_in_method(:PokeBattle_Move, :getSecondaryType, "secondtype = []",
  "attacker.apply_ability_event(:move_subtype, attacker, self) { |m| secondtype.push(m) }")
...
UniLib.insert_in_method_before(:PokeBattle_Battler, :pbTryUseMove, target,
  "self.apply_ability_event(:try_move, self, basemove) {}")
...
UniLib.insert_in_method_before(:PokeBattle_Battler, :pbProcessMoveAgainstTarget, target,
  "user.apply_ability_event(:move_effect, user, target, i, basemove) {}")

# after move effect events
UniLib.insert_in_method(:PokeBattle_Battler, :pbProcessMoveAgainstTarget, target,
  "user.apply_ability_event(:after_move_effect, user, target, i, basemove) {}")
...
UniLib.insert_in_method_before(:PokeBattle_Battle, :__clauses__pbEndOfRoundPhase, "if i.crested == :VESPIQUEN",
  "i.apply_ability_event(:turn_end, i) {}")

# form change handler
if Reborn
  UniLib.insert_in_method(:PokeBattle_Battler, :pbCheckFormRoundEnd, :TAIL,
    "transformed = false
    self.apply_ability_event(:form_change, self, nil) { |m| transformed = !(self.form = m).nil? } unless self.isFainted?
    if transformed
...
UniLib.insert_in_method_before(:PokeBattle_AI, :pbGetMonRoles, "partyRoles.push(monRoles)",
  "mon.apply_ability_event(:roles, self, mon) { |m| (m.is_a?(Array) ? monRoles += m : monRoles.push(m)) if m }")
